...
(async () => {
    ...
    const verifiers = await page.$$eval('.crypt', spans => spans.map(span => span.innerText));
    let jsonified_verifiers = JSON.stringify(verifiers);
    console.log(jsonified_verifiers);
    await browser.close();
